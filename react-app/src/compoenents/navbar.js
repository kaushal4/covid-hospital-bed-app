...
const Navbar = () => {
  ...
  const getClipPath = () => {
    ...
    if (width < 700 && isNavToggled && toggleNavRef.current) {
      if (shouldNavAnimationPlay) {
        setTimeout(() => {
          setShouldNavAnimation(false);
        }, 500);
        return {
          "clip-path": `circle(23px at ${toggleNavRef.current.offsetLeft}px ${
            toggleNavRef.current.offsetTop +
            toggleNavRef.current.offsetHeight / 2
          }px)`,
      ...
      return {
        "clip-path": `circle(23px at ${toggleNavRef.current.offsetLeft}px ${
          toggleNavRef.current.offsetTop + toggleNavRef.current.offsetHeight / 2
        }px)`,
      };
    } else if (toggleNavRef.current) {
      return {
        "clip-path": `circle(300% at ${toggleNavRef.current.offsetLeft}px ${
          toggleNavRef.current.offsetTop + toggleNavRef.current.offsetHeight / 2
        }px)`,
        transition: `clip-path 0.5s ease`,
      };
  ...
  return (
    <nav className="navbar" style={{ ...getClipPath() }}>
      ...
      />
      <a href="#home" onClick={onButtenClickHandler}>HOME</a>
      <a href="#locationStatus" onClick={onButtenClickHandler}>SEARCH BEDS</a>
      <a href="#covid_symptom_test" onClick={onButtenClickHandler}>SYMPTOM TEST</a>
